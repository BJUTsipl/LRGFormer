...
def show_on_report(batch_idx, num_batches,  loss,loss_de,    elapsed):
    """Formats training stats."""

    clear_line()
    dec = int(np.ceil(np.log10(num_batches)))
    print('Batch {:>{dec}d} / {:d} | Avg loss: {:>1.5f}| loss_de: {:>1.5f}| Avg train time / batch: {:d} ms'.format(batch_idx + 1, num_batches, loss, loss_de, int(elapsed), dec=dec))
...
def validation_PSNR(net, val_data_loader, device, category, save_tag=False):
    ...
    for batch_id, val_data in enumerate(val_data_loader):
        ...
        with torch.no_grad():
            ...
            gt = gt.to(device)
            # dehaze = net(haze)
            dehaze  = net.Encoder(haze)
            # dehaze  = net.Decoder(cor, trr)
            # print(query_der)
