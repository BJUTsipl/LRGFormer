...
class FSAS(nn.Module):
    def __init__(self, dim, bias):
        ...
        self.to_hidden = nn.Conv2d(dim, dim * 6, kernel_size=1, bias=bias)#gai linear
        self.to_hidden_dw = nn.Conv2d(dim * 6, dim * 6, kernel_size=3, stride=1, padding=1, groups=dim * 6, bias=bias)

        self.project_out = nn.Conv2d(dim * 2, dim , kernel_size=1, bias=bias)#gai linear

        self.norm = LayerNorm(dim * 2, LayerNorm_type='WithBias')
        self.dwconv = nn.Conv2d(dim * 2, dim * 2, kernel_size=3, stride=1, padding=1, groups=dim)
        self.patch_size = 8
